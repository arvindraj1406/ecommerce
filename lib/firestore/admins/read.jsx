...
import { collection, doc, onSnapshot } from "firebase/firestore"; // Firestore collection and real-time listener
import useSWRSubscription from "swr/subscription"; // SWR's subscription hook for handling real-time data

// Define a custom hook to fetch admin from Firestore
...
export function useAdmin({ email }) {
  // Use SWRSubscription to listen for changes in the "categories" collection
  const { data, error } = useSWRSubscription(
    ["admins", email], // Key for the SWR cache (can include collection path and params)
    ([path, email], { next }) => {
      // Extract the Firestore collection path from the key

      const ref = doc(db, `admins/${email}`); // Get a reference to the "categories" collection
